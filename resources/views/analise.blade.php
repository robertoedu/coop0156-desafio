...
<body class="text-slate-200 min-h-screen flex flex-col font-sans">
    ...
    <main class="flex-grow max-w-6xl mx-auto px-4 py-12 w-full grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <section
            class="lg:col-span-7 glass-panel rounded-3xl p-8 shadow-2xl relative overflow-hidden transition-all duration-300 hover:border-panelBorder">
            ...
            <form id="form-analise" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Renda Mensal -->
                    <div>
                        <label for="renda_mensal" class="block text-sm font-medium text-slate-400 mb-2">Renda Mensal
                            (R$)</label>
                        <input type="number" step="0.01" id="renda_mensal" name="renda_mensal" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Tipo de Crédito -->
                    <div>
                        <label for="tipo_credito" class="block text-sm font-medium text-slate-400 mb-2">Tipo de
                            Crédito</label>
                    ...
                    <!-- Valor Solicitado -->
                    <div>
                        <label for="valor_solicitado" class="block text-sm font-medium text-slate-400 mb-2">Valor
                            Requerido (R$)</label>
                        <input type="number" step="0.01" id="valor_solicitado" name="valor_solicitado" required
        ...
        <section class="lg:col-span-5 space-y-6">
            ...
            <div id="resultado-vazio"
                class="glass-panel rounded-3xl p-8 text-center border-dashed border-2 border-panelBorder flex flex-col items-center justify-center py-20">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-slate-600 mb-4" fill="none"
                    viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                <h3 class="text-lg font-medium text-slate-400">Aguardando Solicitação</h3>
                <p class="text-sm text-slate-500 mt-2 max-w-xs">Preencha os dados do formulário ao lado e solicite a
                    análise para simular as condições.</p>
            </div>

            <!-- Card de Resultado da Análise -->
            <div id="resultado-analise"
                class="glass-panel rounded-3xl p-8 shadow-2xl relative overflow-hidden hidden">
                ...
                <div class="space-y-4 divide-y divide-panelBorder">
                    ...
                    <div id="dados-reprovado" class="pt-4 hidden">
                        <div class="bg-red-500/10 border border-red-500/20 rounded-xl p-4 mt-2">
                            <span class="text-red-400 text-xs block font-semibold uppercase tracking-wider mb-1">Motivo
                                da Recusa</span>
                ...
                <div id="container-contratacao" class="mt-8 pt-6 border-t border-panelBorder hidden">
                    <button id="btn-contratar"
                        ...
                        <span id="txt-contratar">Ver condições da simulação</span>
                        <svg id="loading-spinner-contratar" class="animate-spin h-5 w-5 text-white hidden"
                            xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
                                stroke-width="4"></circle>
                            <path class="opacity-75" fill="currentColor"
                    ...
                    <p class="text-center text-xs text-slate-500 mt-3">Revise as condições do crédito antes de
                        confirmar a contratação.</p>
                </div>
            </div>

            <!-- Card de Contratação Sucesso/Processando -->
            <div id="card-sucesso-contratacao"
                class="glass-panel rounded-3xl p-8 border-emerald-500/30 text-center shadow-2xl relative overflow-hidden hidden">
                <div
                    class="h-16 w-16 bg-emerald-500/10 text-emerald-400 rounded-full flex items-center justify-center mx-auto mb-4">
                ...
                <p class="text-sm text-slate-400 mt-2">A simulação de crédito foi encaminhada com sucesso para a nossa
                    fila de processamento em segundo plano.</p>
                <div
                    class="bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3 mt-4 text-xs text-emerald-400 font-mono">
                    Status: PROCESSANDO_CONTRATACAO
                </div>
                <button onclick="window.location.reload()"
                    class="mt-6 text-sm text-emerald-400 hover:text-emerald-300 font-medium transition-all">
    ...
    <script>
        function mostrarResultado(analise) {
            ...
            status.classList.toggle('text-emerald-400', aprovado);
            status.classList.toggle('text-red-400', !aprovado);

            document.getElementById('dados-aprovado')
                .classList.toggle('hidden', !aprovado);
            ...
            if (aprovado) {
                ...
                document.getElementById('res-taxa').textContent =
                    Number(analise.taxa_juros).toLocaleString('pt-BR', {
                ...
                document.getElementById('res-comprometimento').textContent =
                    ((parcela / renda) * 100).toLocaleString('pt-BR', {
                        minimumFractionDigits: 1,
                        maximumFractionDigits: 1,
                    }) + '%';
            } else {
                document.getElementById('res-motivo').textContent =
                    analise.motivo_rejeicao;
            }

            const containerContratacao = document.getElementById('container-contratacao');
            const botaoSimulacao = document.getElementById('btn-contratar');

            containerContratacao.classList.toggle('hidden', !aprovado);

            botaoSimulacao.onclick = aprovado ? () => {
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const textoBotao = document.getElementById('txt-solicitar');
            const spinner = document.getElementById('loading-spinner');
            const erroAnalise = document.getElementById('erro-analise');
            ...
            function mostrarErro(mensagem) {
                erroAnalise.textContent = mensagem;
                erroAnalise.classList.remove('hidden');
            }
            ...
            form.addEventListener('submit', async (event) => {
                ...
                erroAnalise.textContent = '';
                erroAnalise.classList.add('hidden');

                document.getElementById('resultado-analise').classList.add('hidden');
                document.getElementById('container-contratacao').classList.add('hidden');
                document.getElementById('resultado-vazio').classList.remove('hidden');

                const campos = new FormData(form);

                const dados = {
                    nome: campos.get('nome').trim(),
                    cpf: campos.get('cpf').replace(/\D/g, ''),
                    renda_mensal: Number(campos.get('renda_mensal')),
                    tipo_credito: campos.get('tipo_credito'),
                    valor_solicitado: Number(campos.get('valor_solicitado')),
                };

                botao.disabled = true;
                textoBotao.textContent = 'Analisando...';
                ...
                try {
                    ...
                    if (!response.ok) {
                        const mensagem = response.status === 422 && resultado.errors ?
                            Object.values(resultado.errors).flat().join('\n') :
                            resultado.message || 'Não foi possível realizar a análise.';
                    ...
                    mostrarErro(
                        ...
                        'A solicitação pode ter sido processada; confira a conexão ' +
                        'antes de enviar novamente.'
                    );
                } finally {
                    botao.disabled = false;
                    textoBotao.textContent = 'Solicitar Análise de Crédito';
